 ...
 *****************************************************************************/
 ...
 /*-------------------------------------------------------------------------------------------*/
 void nms(float **anglec,float **magc,float **dd_image,int nrows ,int ncols,int thresh,int thresh2)
 {
 
   
   /* PERFORMS NON-MAXIMUM SUPRESSION TO DETECT EDGES */
 
   int i,j,r,c,edgepoints1=0,edgepoints2=0,last=0,nochange,row,col,count,run;
   FILE *fpy;
   float angle,maxm;
 ...
 int gauss( unsigned char **xc, int nrows, int ncols, float sigma,int scale)
 {
   FILE *fpy;
   ...
   gauss_filter = (float **)matrix(grows, gcols, -2*scale, -2*scale, sizeof(float)); 
   printf("ALLOCATED MEMORY\n");
      
   set = -scale;
   /*  ALLOCATE MEMORY FOR GAUSS SMOOTH */
   printf(" ALLOCATE MEMORY FOR GAUSS SMOOTH\n");
   gauss_smooth = (float **)matrix(nrows-4*set, ncols-4*set, 2*set, 2*set, sizeof(float));
   write_smooth_image = (unsigned char **)matrix(nrows, ncols, 0, 0, sizeof(char));
   ...
   for( i=-(gcols/2); i<= gcols/2; i++)
     {
       for(j=-(grows/2);j<= gcols/2; j++)
         {
           gauss_filter[i][j] = (1/(2*M_PI*sigma*sigma))*exp(-((i*i)+ (j*j))/(2*sigma*sigma));
  ...
  /* INITIALIZE GAUSS SMOOTH*/
           ...
           for( c=set; c<= -set; c++)
             {
               for(r=set;r<= -set; r++)
                 {
                   gauss_smooth[i][j] = gauss_smooth[i][j] + (float) xc[i+c][j+r] * gauss_filter[c][r];
   ...
   fprintf(fpy, "P5\n%d %d\n255\n", ncols, nrows);
   for(i = 0; i < nrows; i++)
     if(fwrite(&write_smooth_image[i][0], sizeof(char), ncols, fpy) != ncols)
       error("can't write the image");
 ...
 void canny(unsigned char **xc, int nrows, int ncols, int thresh,int thresh2,float sigma,int scale)
 {  
  
   int r,c,i,j,set,edgepoints2,last,nochange=1,edgecount,check,count,row,col,run,t1,t2;
   int count_points =0;
   ...
   set = scale;
   fx = (float **)matrix(nrows+4*scale, ncols+4*scale, -2*scale,-2*scale, sizeof(float)); 
   fy = (float **)matrix(nrows+4*scale, ncols+4*scale, -2*scale, -2*scale, sizeof(float)); 
   fxy = (float **)matrix(nrows+4*set, ncols+4*set, -2*set, -2*set, sizeof(float)); 
   fxx = (float **)matrix(nrows+4*set, ncols+4*set, -2*set, -2*set, sizeof(float)); 
   fyy = (float **)matrix(nrows+4*set, ncols+4*set, -2*set, -2*set, sizeof(float)); 
   gradient =(float **)matrix(nrows+2*set, ncols+2*set, -2*set, -2*set, sizeof(float));
   dd_image=(float **)matrix(nrows+4*set, ncols+4*set, -2*set, -2*set, sizeof(float));
   anglec = (float **)matrix(nrows+4*set, ncols+4*set, -2*set, -2*set, sizeof(float));
       ...
       for(j=0;j<nrows;j++)
         {
           ...
           fxy[i][j] = gauss_smooth[i-1][j-1]+gauss_smooth[i+1][j-1] - gauss_smooth[i+1][j+1] - gauss_smooth[i-1][j-1];
           fxx[i][j] = gauss_smooth[i][j+2]- 2*gauss_smooth[i][j] + gauss_smooth[i][j-2];
           fyy[i][j] = gauss_smooth[i+2][j] - 2*gauss_smooth[i][j] + gauss_smooth[i-2][j];
           gradient[i][j] = fx[i][j] * fx[i][j] + fy[i][j]*fy[i][j];
           dd_image[i][j] =(fx[i][j] * fx[i][j] * fxx[i][j] + 2*fx[i][j]*fy[i][j]*fxy[i][j] + fy[i][j]*fy[i][j]* fyy[i][j])/gradient[i][j];
 ...
 int edgemain()
 {
     ...
     case 't':
       ...
       if(str[0] <= '9' && str[0] >= '0')
         ...
         else
         {
            ...
            error("Usage:edge {-roberts,-prewitt,-sobel,-frei} [-skipNMS] [-t thresh_low thresh_high] img ");
            exit(0); 
         }      
   ...
   /* edge_map is the matrix containbing whether a given pixel is an edge or not */
 
   x = (unsigned char **)matrix(nrows+2*scale, ncols+2*scale, -scale, -scale, sizeof(char));  

   printf("MATRICXC SIZE %d\n", sizeof(x[0]));
   printf("--------------------------------------------------------\n");
   printf("Allocated arrays for output image:gradient and magnitude and edge map\n");
