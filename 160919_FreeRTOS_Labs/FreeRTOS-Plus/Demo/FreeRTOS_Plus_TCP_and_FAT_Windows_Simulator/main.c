...
void encryptTask() {
	...
	while (1) {
		...
		if (cannyQueue != 0) {
			if (xQueueReceive(cannyQueue, &recbuf, (TickType_t)10) == pdPASS) {
...
void socketTask() {
	...
	if (edgesocket != FREERTOS_INVALID_SOCKET){
		...
		printf("[SOCKET]: Error in creating socket!\n");
		/* There was insufficient FreeRTOS heap memory available for the socket
		to be created. */
	}
	uint8_t *buffer;
	buffer = (pixel_t*)malloc(sizeof(pixel_t) * xTotalLengthToSend);
	int i;
	for (i = 0; i < xTotalLengthToSend; i++) {
		buffer[i] = xTotalLengthToSend - i;
	}
	...
	if (FreeRTOS_connect(edgesocket, &xRemoteAddress, sizeof(xRemoteAddress)) == 0){
		...
		while (FreeRTOS_recv(edgesocket, buffer, xTotalLengthToSend, 0) >= 0){
			/* Wait for shutdown to complete.  If a receive block time is used then
			this delay will not be necessary as FreeRTOS_recv() will place the RTOS task
			into the Blocked state anyway. */
			vTaskDelay(1);
		}
		printf("[SOCKET]: Socket shut down\n");
		/* The socket has shut down and is safe to close. */
		FreeRTOS_closesocket(edgesocket);
...
TimerHandle_t xCheckTimer;
	...
	***NOTE*** Tasks that use the network are created in the network event hook
	...
	static TaskHandle_t xCannyHandle = NULL;
...
/*-----------------------------------------------------------*/
	...
	static const struct xSERVER_CONFIG xServerConfiguration[] =
	{
				...
				{ eSERVER_HTTP, 	8080, 			12, 		configHTTP_ROOT },
		#endif
