...
int cannymain(){
 /*Load 5 different images to have different computation time*/
	...
	switch (imagestate++) {
	case 1:
		in_bitmap_data = load_bmp("Canny/runway1.bmp", &ih);
		break;
	case 2:
		in_bitmap_data = load_bmp("Canny/runway2.bmp", &ih);
		break;
	case 3:
		in_bitmap_data = load_bmp("Canny/runway3.bmp", &ih);
		break;
	case 4:
		in_bitmap_data = load_bmp("Canny/runway4.bmp", &ih);
		break;
	case 5:
		in_bitmap_data = load_bmp("Canny/runway5.bmp", &ih);
		break;
	default:
		printf("Error could not read such image state\n");
	}
	if (imagestate > 5)
		imagestate = 1;
	...
	//out_bitmap_data is an array of short int of length in the size 262146
	//This array shall be encrypted
    pixel_t *out_bitmap_data = canny_edge_detection(in_bitmap_data, &ih, 20, 80, 1.0);
	int i;

    if (out_bitmap_data == NULL) {
        printf("main: failed canny_edge_detection.\n");
        return 1;
    }

	cannyMessage sendbuf; //This is the filtered image that will be send to encrypt
...
#endif	
	printf("[CANNY]: Sending data to queue...\n");
	if (cannyQueue != 0) {
		if (xQueueSendToBack(cannyQueue, &sendbuf, (TickType_t)10) != pdPASS) {
			printf("Queue is full\n");
		}
	}
	printf("[CANNY]: Data successfully sent on queue!\n");
    if (save_bmp("Canny/out.bmp", &ih, out_bitmap_data)) {
